#ifndef RED_BLACK_TREE_ITERATOR_SET_HPP
# define RED_BLACK_TREE_ITERATOR_SET_HPP
...
namespace ft
{
	...
	// ###########################################################################################################
		public:
			Rbt_iterator_set(): current_node(){}
			Rbt_iterator_set(node_pointer ptr): current_node(ptr){}
			~Rbt_iterator_set(){}

			operator Rbt_iterator_set<const T, node_pointer, Compare>() const 
			{
				return Rbt_iterator_set<const T, node_pointer, Compare>(current_node);
			}
	...
	// ###########################################################################################################
		...
		Rbt_iterator_set operator++(int) // i++
		{
			Rbt_iterator_set it_copy = *this;
			++(*this);
			return(it_copy);
		}
		...
		{
			return (this->current_node->value.first);
		}
		...
		bool operator==(const Rbt_iterator_set& rhs) const
		{
